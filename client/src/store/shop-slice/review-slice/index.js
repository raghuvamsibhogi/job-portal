...
export const addReview = createAsyncThunk('./review/addreview',async({userId,userName,reviewMessage,reviewValue,productId})=>{
    const response = await axios.post('http://localhost:5000/api/shop/review/addReview',{userId,userName,reviewMessage,reviewValue,productId})
    return response.data
})
export const getProductReview = createAsyncThunk('./review/getProductReview',async(productId)=>{
    const response = await axios.get('http://localhost:5000/api/shop/review/getProductReviews',{params:{productId}})
    return response.data
})
