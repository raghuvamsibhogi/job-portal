...
export const addCart = createAsyncThunk('cart/addCart',async({userId,productId,quantity})=>{
    const response = await axios.post('http://localhost:5000/api/shop/cart/addCartItem',{
        userId,productId,quantity
    })
    return response.data
})
export const updateCart = createAsyncThunk('cart/updateCart',async({userId,productId,quantity})=>{
    const response = await axios.put('http://localhost:5000/api/shop/cart/updateCartItem',{
        userId,productId,quantity
    })
    return response.data
})
export const deleteCart = createAsyncThunk('cart/deleteCart',async({userId,productId})=>{
    const response = await axios.delete(`http://localhost:5000/api/shop/cart/deleteCartItem/${userId}/${productId}`)
    return response.data
})
export const getCart = createAsyncThunk('cart/getCart',async({userId})=>{
    const response = await axios.get(`http://localhost:5000/api/shop/cart/getCartItem/${userId}`)
    return response.data
})
export const checkQuantity = createAsyncThunk('cart/checkQuantity',async({userId})=>{
    const response = await axios.get(`http://localhost:5000/api/shop/cart/checkQuantity`,
